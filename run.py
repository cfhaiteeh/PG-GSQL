...
def train(all_pre,model, data, params):
    """ Trains a model.
    ...
    while keep_training:
        ...
        if params.interaction_level:
            epoch_loss = train_epoch_with_interactions(
                ...
                params,
                model,
                randomize=not params.deterministic,epoch=epochs)
        else:
        ...
        train_eval_results_all = eval_fn(training_sample,
                                     model,
                                     params.train_maximum_sql_length,
                                     name=os.path.join(params.logdir, "train-eval"),
                                     write_results=True,
                                     gold_forcing=True,
                                     metrics=TRAIN_EVAL_METRICS,all_pre=all_pre)

        train_eval_results=train_eval_results_all[0]
        ...
        log.put(
                "train first turn "+
                "%.2f" %
                train_eval_results_all[2])
        # Run an evaluation step on the validation set.
        valid_eval_results_all = eval_fn(valid_examples,
                                     model,
                                     params.eval_maximum_sql_length,
                                     name=os.path.join(params.logdir, "valid-eval"),
                                     write_results=True,
                                     gold_forcing=True,
                                     metrics=VALID_EVAL_METRICS,all_pre=all_pre)
        valid_eval_results=valid_eval_results_all[0]
        ...
        average_acc=(string_accuracy)
        
        if not saved and   average_acc > maximum_string_accuracy :
            # maximum_string_tot=tot_acc
            maximum_string_accuracy = average_acc
...
if __name__ == "__main__":
    if os.path.exists('sparc_debug/args.log'):
        os.remove('sparc_debug/args.log')
    main()
