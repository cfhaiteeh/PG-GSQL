...
class UtteranceItem():
    ...
    def histories(self, maximum):
        if maximum > 0:
            ...
            return history_seqs
        return []
    def get_utterance_types(self):
      
        tp=[]
        for x,y in zip(self.interaction.utterances[self.utterance_index].utterance_types,self.interaction.utterances[self.utterance_index].utterance_or):
            tp.append([x,y])
    ...
    def get_utterance_toks(self):
        return self.interaction.utterances[self.utterance_index].utterance_toks
    def get_used_table(self):
        return self.interaction.utterances[self.utterance_index].used_table
    def get_utterance_info(self):
        return self.interaction.utterances[self.utterance_index].utterance_info
    def get_used_column(self):
        
        return self.interaction.utterances[self.utterance_index].used_column
...
class PredUtteranceItem():
    def __init__(self,utterance_toks,utterance_info,ng,utterance_types,utterance_or,used_table,used_column,
                 input_sequence,
                 interaction_item,
                 previous_query,
                 index,
                 available_snippets):
        self.pre_gold_query=ng
        self.input_seq_to_use = input_sequence
        self.interaction_item = interaction_item
        self.utterance_info=utterance_info

        self.index = index
        self.available_snippets = available_snippets
        self.prev_pred_query = previous_query
        self.utterance_toks=utterance_toks
        self.utterance_types=utterance_types
        self.utterance_or=utterance_or

        self.used_table=used_table
        self.used_column=used_column
    def get_pre_gold_query(self):
        return self.pre_gold_query
    def input_sequence(self):
        return self.input_seq_to_use
    def get_utterance_types(self):
  
        tp=[]
        for x,y in zip(self.utterance_types,self.utterance_or):
            tp.append([x,y])
    ...
    def get_utterance_toks(self):
        return self.utterance_toks

    def get_utterance_info(self):
        return self.utterance_info

    def get_used_table(self):
        
        return self.used_table
    def get_used_column(self):
        return self.used_column
    def histories(self, maximum):
        if maximum == 0:
            return histories
        histories = []
...
class InteractionItem():
    ...
    def next_utterance(self):
        ...
        if self.index>0:
            ng=self.gold_query(self.index-1)
        else:
            ng=[]
        self.index += 1
        ...
        return PredUtteranceItem(utterance.utterance_toks,utterance.utterance_info,
        ng,
            utterance.utterance_types,
            utterance.utterance_or,

            utterance.used_table,
            utterance.used_column,
