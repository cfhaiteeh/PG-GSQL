...
class Utterance:
    ...
    def __init__(self,
        ...
        self.utterance_info=example['utterance_info']

        self.utterance_toks=example['utterance_toks']
        self.utterance_types=example['utterance_types']
        self.utterance_lt=example['utterance_lt']
        self.utterance_or=example['utterance_or']
   
        usedTID=schema.usedTID
     
      
       
        usedCID=schema.usedCID
      
        self.used_table =[]
        for i,_x in enumerate(example['used_table']):
            if i in usedTID:
                self.used_table.append(_x)
        self.used_column =[]
        for i,_x in enumerate(example['used_column']):
            if i in usedCID:
                self.used_column.append(_x)
