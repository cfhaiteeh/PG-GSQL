...
class TokenPredictor(torch.nn.Module):
    ...
    def __init__(self, params, vocabulary, attention_key_size):
        ...
        self.attention_module = AttentionAddMode(params.decoder_state_size, attention_key_size, attention_key_size,name='attention_module')
        schema_attention_key_size=300
        self.col_attention_module = AttentionAddMode(300, schema_attention_key_size, schema_attention_key_size,name='col_attention_module')
        self.query_attention_module = AttentionAddMode(300,300, 300,name='query_attention_module')
        self.start_query_attention_vector = torch_utils.add_params((300,), "start_query_attention_vector")
        if params.use_query_attention:
            if self.params.discourse_level_lstm:
                self.in_size=350+300+300+schema_attention_key_size
            else:
                self.in_size=300+300+300+schema_attention_key_size
        else:
            if self.params.discourse_level_lstm:
                self.in_size=350+300+schema_attention_key_size
            else:
                self.in_size=300+300+schema_attention_key_size
        self.state_transform_weights = torch_utils.add_params((self.in_size, 300), "weights-state-transform")
        self.vocabulary_weights = torch_utils.add_params((300,len(vocabulary)), "weights-vocabulary")
...
class SchemaTokenPredictor(TokenPredictor):
    ...
    def __init__(self, params, vocabulary, utterance_attention_key_size, schema_attention_key_size, snippet_size):
        ...
        schema_attention_key_size=300
        self.schema_token_weights = torch_utils.add_params((300, schema_attention_key_size), "weights-schema-token")
        self.previous_token_weights = torch_utils.add_params((300, 300), "weights-previous-token")
        self.state2copyswitch_transform_weights = torch_utils.add_params((950+300, 1), "state2copyswitch_transform_weights")
    ...
    def _get_gen_or_copy_switch(self, state):
        copy_switch = torch.sigmoid(torch_utils.linear_layer(state, self.state2gencopyswitch_transform_weights))
        return copy_switch.squeeze()
    ...
    def _get_previous_token_scorer(self, state):
        scorer = torch.t(torch_utils.linear_layer(state, self.previous_token_weights))
        return scorer
    ...
    def forward(self,previous_final_state,tabok,tbidx, tabs,allTable,IdxCol, prediction_input,tb_col,previous_query_now,previous_query_state_now,decoder_input,schem_map,output_embedder,query_attention_results_dis, dropout_amount=0.):
        ...
        schema_attention_results=self.col_attention_module(decoder_state,schema_v).vector
        if len(previous_query_state_now) > 0:
            query_attention_results = self.query_attention_module(decoder_state, previous_query_state_now)
        else:
        ...
        if self.params.use_query_attention:
            state_and_attn = torch.cat([decoder_state, utterance_attention_results.vector.cuda(),schema_attention_results,query_attention_results.vector], dim=0)
            c_k=torch.cat([utterance_attention_results.vector.cuda(),schema_attention_results,query_attention_results.vector],dim=0)
        else:
        ...
        if  len(previous_query_state_now) > 0:
            query_attention_weight=query_attention_results.distribution.squeeze(0)
            switch = self._get_copy_switch(state_and_attn)
        gen_switch=None
        intermediate_state = self._get_intermediate_state(state_and_attn, dropout_amount=dropout_amount)
        vocab_scores, vocab_tokens = self._score_vocabulary_tokens(intermediate_state)
