...
def get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t, hds, max_seq_length, num_out_layers_n=1, num_out_layers_h=1):
    ...
    all_encoder_layer, _, _, i_nlu, i_hds,\
    l_n, l_hpu, l_hs, \
...
def prepare_input(tokenizer, input_sequence, input_schema, max_seq_length):
    ...
    all_hds=[]
    for kk in input_schema.column_names_embedder_input:
        if '. table' in kk or kk=='column *':
            all_hds.append(kk)
    ...
    for hds1 in all_hds:
        ...
        _, segment_ids1, _, _, _ = generate_inputs(tokenizer, nlu_tt1, new_hds1)
...
def prepare_input_v2(tokenizer, input_sequence, input_schema):
    ...
    for hds1 in all_hds:
        ...
        if hds1_table == current_table:
            ...
            _, segment_ids1, _, _, _ = generate_inputs(tokenizer, nlu_tt1, current_hds1)
...
def get_bert_encoding(bert_config, model_bert, tokenizer, input_sequence, input_schema, bert_input_version='v2', max_seq_length=512, num_out_layers_n=1, num_out_layers_h=1):
   
    nlu_t, hds = prepare_input(tokenizer, input_sequence, input_schema, max_seq_length)

    wemb_n, wemb_h, l_n, l_hpu, _, _, t_to_tt_idx, _, t_to_tt_idx_hds = get_wemb_bert(bert_config, model_bert, tokenizer, nlu_t, hds, max_seq_length, num_out_layers_n, num_out_layers_h)
