...
def compute_loss(gold_seq,
                 scores,
                 index_to_token_maps,
                 gold_tok_to_id,gold_match_id,pre_len,
                 noise=0.00000001):
...
def encode_sequence(utterance_toks,utterance_types,  rnns, embedder, dropout_amount=0.,model='utterance',schema_embedding =None):
...
def encode_sequence1(sequence, rnns, embedder, dropout_amount=0.):
    """ Encodes a sequence given RNN cells and an embedding function.
    Inputs:
        seq (list of str): The sequence to encode.
        rnns (list of dy._RNNBuilder): The RNNs to use.
