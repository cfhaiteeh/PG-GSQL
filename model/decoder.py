...
class SequencePredictorWithSchema(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.isused_embedder=isused_embedder
        self.lstms = torch_utils.create_multilayer_lstm_params(params.decoder_num_layers,self.in_size ,300, "LSTM-d")
        self.token_predictor = token_predictor
        self.output_embedder = output_embedder
        self.column_name_token_embedder = column_name_token_embedder
        self.start_token_embedding = torch_utils.add_params((300,), "y-0")
    ...
    def forward(self,
        ...
        ocols_val = [x[1].lower() for x in input_schema.column_names_original]
        ...
        primary_keys=input_schema.primary_keys
            ...
            if len(sequence) == 0 or sequence[-1] != EOS_TOK:
                ...
                if (flag==1 or len(table_id)==0) :
                    ...
                    if len(previous_query_states) > 0:
                        previous_query_state_all = previous_query_states[-1]
                        previous_tokens_all=previous_queries[-1]
                        previous_query_now=[]
                        previous_query_state_now=[]
                        for xx,yy in zip(previous_tokens_all,previous_query_state_all):
                            if xx in self.voc or (len(input_tab_emb)>0 and xx in input_tab_tks):
                                previous_query_now.append(xx)
                                previous_query_state_now.append(yy)
                    ...
                    if len(previous_query_states) > 0:
                        ...
                        previous_query_state_now=[]
                        for xx,yy in zip(previous_tokens_all,previous_query_state_all):
                            if xx in self.voc or (len(col_emb)>0 and xx in col_tks):
                                previous_query_now.append(xx)
                                previous_query_state_now.append(yy)
                ...
                prediction = self.token_predictor(previous_final_state,tabok,tbidx,tables,allTable,IdxCol,prediction_input,next_cols,previous_query_now,previous_query_state_now,decoder_input,input_schema.column_names_surface_form_to_id,self.output_embedder,query_attention_results_dis, dropout_amount=dropout_amount)
                ...
                if gold_sequence:
                    ...
                    if from_switch is not None:
                        ...
                        query_attention_weight=prediction.query_attention_weight*(1.0-from_switch)
                    ...
                    argmax_token,argmax_index=add_util.common_error_revise(primary_keys,argmax_token,argmax_index,sequence,input_sequence,previous_queries,probabilities,distribution_map,allCol)
                    if len(previous_queries)!=0:
                        previous_tokens_all=previous_queries[-1]
                        if 'order' in previous_tokens_all:
                            while argmax_token=='max' or argmax_token=='min':
                                probabilities[argmax_index]=0.0
                                argmax_index = int(np.argmax(probabilities))
                                argmax_token = distribution_map[argmax_index]
                    if len(sequence)>=2 and sequence[-1]=='(' and sequence[-2]=='count':
                        if argmax_token !='*':
                            cf=0
                            for xql in input_sequences:
                                for xq in xql:
                                    if xq.lower()=='different' or xq.lower()=='DISTINCT'.lower():
                                        cf=1
                                        break
                                if cf==1:
                                    break
                    ...
                    for xc in input_sequence:
                        if xc.lower()=='different' or xc.lower()=='DISTINCT'.lower():
                            if argmax_token=='*' and sequence[-1]=='select'  :
                                probabilities[argmax_index]=0.0
                                argmax_index = int(np.argmax(probabilities))
                                argmax_token = distribution_map[argmax_index]
                            break

                    if len(sequence)>=2 and sequence[-1]=='by' and sequence[-2]=='group':
                            while argmax_token  =='name' :
                                ncs=0
                                for idx,ov in enumerate(ocols_val):
                                    if ov == argmax_token and idx in primary_keys:
                                        ncs=1
                                        break
                                if ncs==1:
                                    break
        ...
        return SQLPrediction(predictions,
                             sequence,
                             probability),now_dict,switch_list,query_attention_weight_list,query_tks_list,question_attention_weight_list,cur_input_tks_list
